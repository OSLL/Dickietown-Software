...
class VehicleCoordinator():
    ...
    def __init__(self):
        ...
        if rospy.get_param("~intersectionType") == "trafficLight":
            ...
            self.possible_intersection_type = self.TR_L
        elif rospy.get_param("~intersectionType") == "plain":
            self.traffic_light_intersection = False
            self.possible_intersection_type = self.SIMPLE
        else:
            self.traffic_light_intersection = False
            self.possible_intersection_type = self.ANY
    ...
    def process_signals_detection(self, msg):
        rospy.loginfo('[simple_coordination_node] signals arrived!: msg.traffic_light_state:%s;  msg.right: %s; msg.front: %s;', msg.traffic_light_state, msg.right, msg.front)
    ...
    def reconsider(self):
        if self.state == State.LANE_FOLLOWING:
            ...
            rospy.loginfo('[simple_coordination_node] self.right_veh: %s; self.opposite_veh: %s;', self.right_veh, self.opposite_veh)
